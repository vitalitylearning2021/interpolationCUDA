...
a function by properly combining its samples.  
The *interpolation points* are the points where the function needs to be
reconstructed and are located within the region covered by the function
samples, as in Figure [1.1](#interpolationProblem). Thanks to
interpolation, the properties of a function are “coded” within a certain
number of properly chosen samples. The problem of reconstructing a
function in the region outside that covered by the samples is called
extrapolation. Extrapolation requires techniques completely different
from those used for interpolation and is not dealt with here.  
Figure [1.1](#interpolationProblem) below shows an example of the
...
In other words, image inpainting enables reconstructing the image at the
...
corresponding index. In C/C++ and CUDA, the value of \(\lfloor x\rceil\)
can be computed by `round(x)`.  
Interpolation is typically a polynomial approximation of a function. The
\(^{(0)}\) superscript in \(f^{(0)}(x)\) indicates that nearest-neighbor
interpolation exploits a polynomial of degree \(0\).
